...
// exec: sed -e 's/^c-library\( .*\)/cs-vocabulary jni\n\nget-current also jni definitions\n\nc-library\1/g' -e 's/^end-c-library/end-c-library\nprevious set-current/g' -e s/JNINativeInterface-/JNIEnv-/g -e s/JNIInvokeInterface-/JavaVM-/g -e 's/\(c-function .*\)/\\ \1/g' -e 's/\(ReleaseStringUTFChars.*\) a a s/\1 a a a/g' -e 's/s" c" add-lib/s" ((struct JNI:*(Cell*)(x.spx\[arg0])" ptr-declare $+\[]!/g' -e 's/-- s/-- a/g' | grep -v '\.\.\. --'
