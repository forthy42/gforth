...
\ ------------------------------------------------------------------------------
\ Version 0.11 8 April 2015 Tests for REPLACES SUBSTITUTE UNESCAPE added
...
T{ S" abcd" buf 3 SUBSTITUTE NIP NIP 0< -> TRUE }T           \ Just too long
T{ S" abcdef" buf 6 SUBSTITUTE S" abcdef" $checkn -> FALSE 0 }T
T{ S" abcdef" buf SWAP MOVE buf 6 buf 20 SUBSTITUTE -ROT
   S" abcdef" $check 0= OVER 0= AND SWAP 0< OR -> TRUE }T    \ c-addr1 = c-addr2

\ Definition using a name on the stack
: $create  ( caddr u -- )
   S" name" REPLACES          ( -- )
   S" create %name%" buf 40 SUBSTITUTE \ Using PAD is not a good idea
   if EVALUATE then
;
