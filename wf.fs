...
\ Copyright (C) 2003,2004,2005,2006,2007,2008,2010,2013,2014,2015,2019,2020,2021,2022,2023,2025 Free Software Foundation, Inc.
...
: maintainer ( -- )
    '<' parse -trailing mail-name $! '>' parse mail $! ;
: pgp-key ( -- )
...
: favicon
    parse-name _favicon $! ;
: expands '#' parse expand-prefix $! parse-name expand-postfix $! ;

icons icons

Variable style$
: style> style$ @ 0= IF  s" " style$ $!  THEN  style$ $@ tag-option $! ;
: >style tag-option $@ style$ $! tag-option $free ;
